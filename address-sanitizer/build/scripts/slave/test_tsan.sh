...
make clean && \
OMIT_DYNAMIC_ANNOTATIONS_IMPL=1 LIBS=$LIBTSAN_A make l64 -j16 CC=clang CXX=clang++ LDOPT="-pie -ldl -Wl,--whole-archive $LIBTSAN_A -Wl,--no-whole-archive" OMIT_CPP0X=1 EXTRA_CFLAGS="$EXTRA_COMPILER_FLAGS" EXTRA_CXXFLAGS="$EXTRA_COMPILER_FLAGS" && \
